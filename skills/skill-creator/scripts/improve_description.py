...
by calling `opencode -p` as a subprocess (same auth pattern as run_eval.py —
uses the session's OpenCode auth, no separate ANTHROPIC_API_KEY needed).
"""
...
def _call_opencode(prompt: str, model: str | None, timeout: int = 300) -> str:
    """Run `opencode -p` with the prompt on stdin and return the text response.
    ...
    # Remove OPENCODE env var to allow nesting opencode -p inside a
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"opencode -p exited {result.returncode}\nstderr: {result.stderr}"
        )
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # SDK path did this as a true multi-turn; `opencode -p` is one-shot, so we
